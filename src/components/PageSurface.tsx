...
export function PageSurface({
  ...
  const external = source === 'url' && typeof page?.backgroundURL === 'string'
    ? page.backgroundURL
    : source === 'saved'
      ? saved?.externalURL
      : null
  ...
  } else if (!suppressMedia && external && /^(https?:\/\/|\/)/i.test(external)) {
    style.backgroundImage = `url("${String(external).replace(/["'()]/g, '')}")`
  }

  const externalVideo = !suppressMedia && external && /\.(mp4|webm)(\?|$)/i.test(external) ? external : null

  return (
    <div className={className} style={style}>
      {!suppressMedia && (desktopImage || resolvedMobileImage) && (
      ...
      {!suppressMedia && (videoURL || externalVideo) && (
        <video
