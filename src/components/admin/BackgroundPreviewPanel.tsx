...
const mediaURL = (media: Media | null | undefined) => {
  ...
  return media.externalURL || media.url || media.sizes?.hero?.url || (media.id ? `/api/media-file/${media.id}` : null)
}

const backgroundIDFromURL = () => {
  if (typeof window === 'undefined') return null
  const match = window.location.pathname.match(/\/backgrounds\/(\d+)(?:\/|$)/)
...
export default function BackgroundPreviewPanel() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [record, health] = await Promise.all([
        ...
        fetch(`/api/media-doctor?background=${id}`, { credentials: 'include', cache: 'no-store' }).then((response) => response.json()).catch(() => null),
      ])
      if (!active) return
  ...
  const frames = useMemo(() => {
    ...
    const selected = device === 'mobile' && background.mobileFrames?.length ? background.mobileFrames : background.desktopFrames || background.mobileFrames || []
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setFrame((current) => (current + 1) % frames.length), 850)
  ...
  return (
    <section className="background-preview-panel">
      <div className="background-preview-panel__heading">
        <div><strong>Vista previa y Media Doctor</strong><p>Comprueba el resultado real antes de guardar o publicar. La secuencia se reproduce aquí y en la página Inicio.</p></div>
        <a href="/" target="_blank" rel="noreferrer">Abrir Inicio ↗</a>
      </div>
      {!id && <p className="background-preview-panel__empty">Guarda primero este background para activar su vista previa y diagnóstico.</p>}
      ...
      {!loading && id && <>
        <div className="background-preview-panel__toolbar">
          ...
          <span>{background?.name || 'Background'} · {frames.length ? `frame ${String(Math.min(frame + 1, frames.length)).padStart(2, '0')} / ${frames.length}` : 'imagen única'}</span>
        </div>
        <div className={`background-preview-panel__stage ${device === 'mobile' ? 'is-mobile' : ''}`}>
          {source ? <img src={source} alt="Vista previa del background" onError={(event) => { event.currentTarget.dataset.error = 'true' }} /> : <div className="background-preview-panel__empty">No hay un archivo reproducible asignado todavía.</div>}
      ...
      <style jsx>{`
        ...
        @media(max-width:720px){.background-preview-panel__heading{flex-direction:column}.background-preview-panel__heading a{width:100%;text-align:center}.background-preview-panel__toolbar span{width:100%;margin-left:0}.background-preview-panel__stage{min-height:210px}.background-preview-panel__stage img{min-height:210px}}
      `}</style>
    </section>
  )
}
